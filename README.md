# Hi, I'm Pani Chendra 👋

🎓 B.Tech IT @ SASTRA University (2023–2027)  
💻 Full Stack Developer (MERN) | Machine Learning Enthusiast  
📍 Passionate about building real-world intelligent systems  

---

## 🚀 About Me

- Strong interest in **Machine Learning & Full Stack Development**
- Built and deployed an **AI-based Retinal Disease Classification System**
- Love working on **real-world problems using scalable systems**
- Currently preparing for **placements + improving DSA & system design**

---

## 🔬 Featured Projects

### 🧠 Retinal OCT Classification System
- Built using **EfficientNet-B0 + CBAM Attention**
- Trained on **60,000+ retinal images**
- Achieved **~97% accuracy**
- Integrated **Grad-CAM** for explainable AI
- Deployed using **Gradio on Hugging Face Spaces**

🔗 [Live Demo](https://huggingface.co/spaces/PaniChendra/retinal-oct-classifier)

---

### 🌐 CampusClaim – Lost & Found Platform
- Full-stack **MERN application**
- Secure authentication using **JWT**
- Image upload using **Multer**
- REST APIs for item reporting & claiming
- Responsive UI using React

🔗 [Live Demo](https://campus-claim-frontend-nine.vercel.app/)

---

## 🛠 Tech Stack

**Languages:**  
C++, Python, JavaScript  

**Full Stack:**  
MongoDB, Express.js, React.js, Node.js  

**Machine Learning:**  
CNNs, Transfer Learning, CBAM, Grad-CAM  

**Tools:**  
PyTorch, Scikit-learn, OpenCV, Gradio, Git, GitHub  

---

## 📊 Current Focus

- Solving **Data Structures & Algorithms**
- Improving **system design & backend scalability**
- Building **advanced ML + Full Stack projects**

---

## 📫 Connect With Me

📧 Email: 127015067@sastra.ac.in  
🔗 LinkedIn: https://www.linkedin.com/in/pani-chendra-48b0442b6  

---

⭐ *Always learning, always building*
